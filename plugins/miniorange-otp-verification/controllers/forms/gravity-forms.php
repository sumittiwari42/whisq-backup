...
function get_gf_form_list($gf_otp_enabled,$disabled,$key)
{
	...
	if(!MoUtility::isBlank($gf_otp_enabled))
	{
		foreach ($gf_otp_enabled as $gfkey=>$gf) 
		{
			echo '<div id="gfrow'.$key.'_'.$keyunter.'">
					'.__( "Form ID", 'wp-otp-verification').': <input class="field_data" id="gravity_form_'.$key.'_'.$keyunter.'" name="gravity_form['.$key.'][]" type="text" value="'.$gf.'">&nbsp';
			echo '</div>';
			$keyunter+=1;
		}
	}
	else
	{
		echo '<div id="gfrow'.$key.'_0"> 
				'.__( "Form ID", 'wp-otp-verification').': <input id="gravity_form_'.$key.'_0" class="field_data"  name="gravity_form['.$key.'][]" type="text" value="">&nbsp';
		echo '</div>';
	}
