<?php
	...
	class WooCommerceRegistrationForm extends FormInterface
	{
		...
		{
			return get_option('mo_customer_validation_wc_restrict_duplicates') ? true : false;
		}
		...
		function mo_add_phone_field()
		{
			echo '<p class="form-row form-row-wide">
					<label for="reg_billing_phone">'.__('Phone', 'wp-otp-verification').'<span class="required">*</span></label>
		...
		function isPhoneNumberAlreadyInUse($phone,$key)
		{
			...
			$results = $wpdb->get_row("SELECT `user_id` FROM `wp_usermeta` WHERE `meta_key` = '$key' AND `meta_value` =  '$phone'");			
		...
		function handleFormOptions()
		{
			...
			update_option('mo_customer_validation_wc_restrict_duplicates',
