<?php
	...
	class WooCommerceCheckOutForm extends FormInterface
	{
		...
		function handleForm()
		{
			...
			$this->paymentMethods 	= maybe_unserialize(get_option('mo_customer_validation_wc_checkout_payment_type'));
		...
		function my_custom_checkout_field( $checkout )
		{
	        ...
	        'label'         => __('Verify Code','wp-otp-verification'),
	        'required'  	=> true,
	        'placeholder'   => __('Enter Verification Code','wp-otp-verification'),
		...
		{
			if(strcasecmp($this->otpType,self::TYPE_PHONE)==0)
				echo '<div title="'.__("Please Enter a Phone Number to enable this link").'"><a href="#" style="text-align:center;color:grey;pointer-events:none;display:none;" id="miniorange_otp_token_submit" class="" >'.__("[ Click here to verify your Purchase ]",'wp-otp-verification').'</a></div>';
			else
				echo '<div title="'.__("Please Enter an Email Address to enable this link").'"><a href="#" style="text-align:center;color:grey;pointer-events:none;display:none;" id="miniorange_otp_token_submit" class="" >'.__("[ Click here to verify your Purchase ]",'wp-otp-verification').'</a></div>';
		}
		...
		function showButtonOnPage()
		{
			if(strcasecmp($this->otpType,self::TYPE_PHONE)==0)
				echo '<input type="button" class="button alt" style="'
					...
					.__("Please Enter a Phone Number to enable this.",'wp-otp-verification').'" value="'
					.__("Place order",'wp-otp-verification').'"></input>';
			else
				echo '<input type="button" class="button alt" style="'
					. ( $this->popupEnabled ? 'float: right;line-height: 1;margin-right: 2em;padding: 1em 2em; display:none;' : 'display:none;width:100%' )
					.'" id="miniorange_otp_token_submit" disabled title="'
					.__("Please Enter an Email Address to enable this.",'wp-otp-verification').'" value="'
					.__("Place order",'wp-otp-verification').'"></input>';
		}
		...
		function isPaymentVerificationNeeded()
		{
			...
			return array_key_exists($payment_method,$this->paymentMethods);
		}
		...
		{
			wp_register_script( 'wccheckout', MOV_URL . 'includes/js/wccheckout.min.js?version='.MOV_VERSION , array('jquery') ,'1.0',true);
		...
		function handleFormOptions()
		{
			...
			update_option('mo_customer_validation_wc_checkout_payment_type',maybe_serialize($paymentMethods));
		}
	}
