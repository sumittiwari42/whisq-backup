<?php
	...
	class WPLoginForm extends FormInterface
	{
		...
		function _handle_mo_wp_login($user,$username,$password)
		{
			...
			$phone_number = MoUtility::processPhoneNumber(get_user_meta($user->data->ID, $this->phoneNumberKey));
		...
		function askPhoneAndStartVerification($user,$key,$username,$phone_number)
		{
			...
			{		
				...
				miniorange_site_challenge_otp(NULL,$user->data->user_login,NULL,NULL,'external',NULL,
					array('data'=>array('user_login'=>$username),'message'=>__(MoMessages::showMessage('REGISTER_PHONE_LOGIN'),
					'wp-otp-verification'),'form'=>$key,'curl'=>MoUtility::currentPageUrl()));
			}					
		}
