<?php
	...
	class FormCraftBasicForm extends FormInterface
	{
		...
		function handleForm()
		{
			...
			$this->otpType = get_option('mo_customer_validation_formcraft_enable_type');
			$this->formCraftForms = maybe_unserialize(get_option('mo_customer_validation_formcraft_otp_enabled'));
		...
		{
			return get_option('mo_customer_validation_formcraft_enable') ? TRUE : FALSE;
		}
		...
		function validate_formcraft_form_submit()
		{
			...
			if($this->otpType==self::TYPE_PHONE 
				...
				$this->sendJSONErrorMessage(array( "errors" => array( $this->formCraftForms[$id]['phonekey'] 
					=> __(MoMessages::showMessage('PHONE_MISMATCH'), 'wp-otp-verification'))));
			if($this->otpType==self::TYPE_EMAIL 
				...
				$this->sendJSONErrorMessage(array( "errors" => array( $this->formCraftForms[$id]['emailkey'] 
					=>  __(MoMessages::showMessage('EMAIL_MISMATCH'), 'wp-otp-verification'))));
			if(MoUtility::isBlank($_POST[$formdata['verifyKey']]))
				$this->sendJSONErrorMessage(array( "errors" => array( $this->formCraftForms[$id]['verifyKey'] 
					=>  __(MoMessages::showMessage('INVALID_OTP'), 'wp-otp-verification'))));
		...
		function checkIfVerificationNotStarted($errors,$id)
		{
			...
			if($this->otpType==self::TYPE_PHONE)
				$this->sendJSONErrorMessage( array( "errors" => array( $this->formCraftForms[$id]['phonekey'] 
					=>  __(MoMessages::showMessage('PLEASE_VALIDATE'),'wp-otp-verification') )));
			else
				$this->sendJSONErrorMessage( array( "errors" => array( $this->formCraftForms[$id]['emailkey'] 
					=>  __(MoMessages::showMessage('PLEASE_VALIDATE'),'wp-otp-verification') )));
		}
		...
		function getEmailFieldId($data,$key,$formData)
		{
			foreach ($formData as $form)
				if($form['elementDefaults']['main_label']==$data['formcraft']['emailkey'][$key]) return $form['identifier'];
			return NULL;
		}

		function getPhoneFieldId($data,$key,$formData)
		{
			foreach ($formData as $form)
				if($form['elementDefaults']['main_label']==$data['formcraft']['phonekey'][$key]) return $form['identifier'];
			return NULL;	
		}

		function getVerificationFieldId($data,$key,$formData)
		{
			foreach ($formData as $form)
				if($form['elementDefaults']['main_label']==$data['formcraft']['verifyKey'][$key]) return $form['identifier'];
			return NULL;
		}
		...
		function handleFormOptions()
		{
			...
			foreach (array_filter($_POST['formcraft']['form']) as $key => $value)
			{
				...
				$form[$value]= array(
					'emailkey'=> $this->getEmailFieldId($_POST,$key,$formData),
					'phonekey'=> $this->getPhoneFieldId($_POST,$key,$formData),
					'verifyKey'=> $this->getVerificationFieldId($_POST,$key,$formData),
