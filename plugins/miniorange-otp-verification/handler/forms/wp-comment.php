<?php
	...
	class WordPressComments extends FormInterface
	{
		...
		function handleForm()
		{
			$this->otpType = get_option('mo_customer_validation_wpcomment_enable_type');

			if(get_option('mo_customer_validation_wpcomment_enable_for_loggedin_users')) {
		...
		{
			return get_option('mo_customer_validation_wpcomment_enable') ? TRUE : FALSE;
		}

		function extend_comment_add_meta_box() 
		{
		    add_meta_box( 'title', __( 'Extra Fields' , 'wp-otp-verification'), array($this,'extend_comment_meta_box')
		    			, 'comment', 'normal', 'high' );
		}
		...
		function extend_comment_meta_box ( $comment ) 
		{
		    ...
		    echo '<table class="form-table editcomment">
					<tbody>
					<tr>
						<td class="first"><label for="phone">'.__( 'Phone', 'wp-otp-verification' ).':</label></td>
		...
		function verify_comment_meta_data( $commentdata ) 
		{
			MoUtility::checkSession();
		  	if ( ! isset( $_POST['phone'] ) )
		  		wp_die( __( 'Error: You did not add a phone number. Hit the Back button on your 
		  					Web browser and resubmit your comment with a phone number.' , 'wp-otp-verification') );
		  	if ( ! isset( $_POST['verifyotp'] ) )
		  		wp_die( __( 'Error: You did not add a Verification Code. Hit the Back button on your 
		  					Web browser and resubmit your comment with a verification code.' , 'wp-otp-verification') );

		  	if(!array_key_exists($this->formSessionVar,$_SESSION))
		  		wp_die(__(MoMessages::showMessage('PLEASE_VALIDATE'),'wp-otp-verification') );

		  	if(array_key_exists($this->formEmailVer, $_SESSION) 
				&& strcasecmp($_SESSION[$this->formEmailVer], $_POST['email'])!=0)
				 wp_die(__(MoMessages::showMessage('EMAIL_MISMATCH'), 'wp-otp-verification'));

			if(array_key_exists($this->formPhoneVer, $_SESSION) 
				&& strcasecmp($_SESSION[$this->formPhoneVer], $_POST['phone'])!=0)
				wp_die(__(MoMessages::showMessage('PHONE_MISMATCH'), 'wp-otp-verification'));
