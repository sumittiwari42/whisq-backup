		...
		function validateFormPost($result, $tag)
		{
			...
			if ( 'text' == $tag->basetype && $name=='email_verify' || 'text' == $tag->basetype && $name=='phone_verify') 
			{
				$_SESSION[$this->formSessionTagName] = $name;
					...
					if($this->checkIfVerificationNotStarted()) $result->invalidate( $tag, __(MoMessages::showMessage('PLEASE_VALIDATE'),'wp-otp-verification') );
					//check if the email being sent is the same email that was verified
					if($this->processEmail()) $result->invalidate( $tag, __(MoMessages::showMessage('EMAIL_MISMATCH'), 'wp-otp-verification') );
					//check if the phone being sent is the same phone that was verified
					if($this->processPhoneNumber()) $result->invalidate( $tag, __(MoMessages::showMessage('PHONE_MISMATCH'), 'wp-otp-verification') ); 
