...
class EmailLogic extends LogicInterface
{
	...
	{
		$message = get_option("mo_otp_success_email_message") ? _x(get_option('mo_otp_success_email_message'),null,'wp-otp-verification') 
														: _x(MoMessages::showMessage('OTP_SENT_EMAIL'),null,'wp-otp-verification');
		return MoUtility::_is_polylang_installed() ? pll__($message) : $message;
	}

	public function _get_otp_sent_failed_message()
	{
		$message = get_option("mo_otp_error_email_message") ? _x(get_option('mo_otp_error_email_message'),null,'wp-otp-verification') 
														: _x(MoMessages::showMessage('ERROR_OTP_EMAIL'),null,'wp-otp-verification');
		return MoUtility::_is_polylang_installed() ? pll__($message) : $message;
	}
	...
	{
		$message = get_option("mo_otp_blocked_email_message") ? _x(get_option('mo_otp_blocked_email_message'),null,'wp-otp-verification') 
														: _x(MoMessages::showMessage('ERROR_EMAIL_BLOCKED'),null,'wp-otp-verification');
		return MoUtility::_is_polylang_installed() ? pll__($message) : $message;
	}
