...
class WC_Cancel_Order{
    ...
    function add_wc_cancel_my_account_orders_status($actions, $order)    {
        ...
        if ($the_order->has_status(array('on-hold', 'pending', 'processing')) && !$item_count['item_count']) {
            $actions['cancelled'] = array('url' => wp_nonce_url(admin_url('admin-ajax.php?action=mark_order_as_cancell_request&order_id=' . $order->id), 'woocommerce-mark-order-cancell-request-myaccount'), 'name' => __('Cancel', 'woocommerce'), 'action' => "cancel-request");
        }
